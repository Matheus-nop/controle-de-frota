    ...
    (async () => {
      ...
      const [rot, eu] = await Promise.all([
        supabase
          ...
          .order("saida_em"),
        user
          ? supabase.from("tecnicos").select("nome, papel").eq("user_id", user.id).maybeSingle()
    ...
    <main style={{ minHeight: "100vh", background: "#EBEEF4" }}>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: 20 }}>
        <h1 style={{ fontSize: 22, margin: "4px 0 2px" }}>
          {nome ? `Olá, ${nome.split(" ")[0]}!` : "Bom trabalho!"} 👷
        </h1>
        ...
        <div style={{ marginTop: 28, textAlign: "center", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <form action="/auth/signout" method="post">
            <button type="submit" style={{ background: "none", border: "none", color: "#8591A5", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
              Sair
            </button>
          </form>
        </div>
